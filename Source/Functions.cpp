...
void notify_center(char* fmt, char* txt, ...)
{
	...
	if ((IsKeyPressed(VK_F8))) {
		...
		GRAPHICS::BEGIN_SCALEFORM_MOVIE_METHOD(handle, "SHOW_SHARD_WASTED_MP_MESSAGE");
		GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_STRING(txt);
		GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_STRING(fmt);
		GRAPHICS::_ADD_SCALEFORM_MOVIE_METHOD_PARAMETER_INT(5);
		GRAPHICS::END_SCALEFORM_MOVIE_METHOD();
	}
}
void notify_center(std::string str) { notify_center(&str[0]); }